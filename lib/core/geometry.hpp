    ...
    namespace core {
        namespace geometry {
            ...
            */
            ...
            auto distsq(const point &p1, const point &p2) -> float {
                return (p1 - p2).norm();
            }
            ...
            */
            auto face_toward(const vector &v1, const normal &n2) -> normal {
                return (dot(v1, n2) < 0.0f) ? (normal)(-v1) : (normal)(v1);
            }
            ...
            */
            auto face_toward(const vector &v1, const vector &v2) -> normal {
                return (dot(v1, v2) < 0.0f) ? (normal)(-v1) : (normal)(v1);
            }
            ...
            (const bounding_box &b, const point &p) -> bounding_box {
                ...
                returning_box.min.x = fmin(b.min.x, p.x);
                returning_box.min.y = fmin(b.min.y, p.y);
                returning_box.min.z = fmin(b.min.z, p.z);

                returning_box.max.x = fmax(b.max.x, p.x);
                returning_box.max.y = fmax(b.max.y, p.y);
                returning_box.max.z = fmax(b.max.z, p.z);
