...
public:
    ...
    /*! \brief Constructor to enclose a single point around a
        bounding box. */
    bounding_box(const point &p) : min(p), max(p) { }
    ...
    auto bb_linear_interpolate(float tx, float fy, float tz);
    ...
    auto relative_position(const point &p) -> vector; 
    ...
    auto operator[](std::size_t i) const -> const point&;

    /*! \brief bounding box set-value operator */
    auto operator[](std::size_t i) -> point&;
